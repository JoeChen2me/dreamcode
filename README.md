...
- 新增记忆卡片 LaTeX 公式渲染支持（KaTeX）
- 新增快捷键切换卡片：Cmd/Ctrl+1~9 直达指定卡片，Cmd/Ctrl+R 返回主页面
- 新增"关于"页面：版本信息、检查更新、自动更新选项（Windows 自动下载安装，macOS 跳转 GitHub Release）
- 修复窗口最小化（隐藏）和最大化（切换）按钮功能
- 优化代码块显示：强制在窗口宽度边界自动换行，避免横向溢出
- 记忆卡片图标更换为 StickyNote

### v1.2.0

- 新增记忆卡片页面，支持 Markdown 阅读与编辑双模式切换
- 新增卡片分组能力：多卡片管理、命名、默认卡片设置、快捷键直达
- 新增卡片管理交互：新建后自动聚焦标题、删除二次确认
- 新增卡片排序能力：手柄拖拽、占位高亮动画、半透明拖拽浮层
